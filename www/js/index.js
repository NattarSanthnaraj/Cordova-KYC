...
function startLiveness(withFace = false) {
    var accuraConfs = {with_face: withFace, app_orientation: screen.orientation.type};
    var config = {
        feedbackTextSize: 18,
    ...
    accura.startLiveness(accuraConfs, config, function (result) {
        if (result.hasOwnProperty('status')) {
            if (result.status) {
                ...
                html += '<h3>Liveness Score: ' + result.score + '</h3>';
                $('#fm-modal .modal-body').empty().append(
                    html
                );
                $('#fm-modal').modal();
            }
...
function startFaceMatch(withFace = false) {
    var accuraConfs = {with_face: withFace, face_uri: facematchURI, app_orientation: screen.orientation.type};
    var config = {
        ...
        if (result.hasOwnProperty('status')) {
            if (result.status) {
                ...
                if (result.with_face) {
                    ...
                    $('#face-div').addClass('justify-content-between');
                    $('#detect-face').fadeIn();
                    getImage('detect-face', result.img_1);
                    $('#score-div').fadeIn('fast', function () {
                        $('#score-div').css('display', 'flex');
                    });
                    return;
                ...
                $('#fm-modal .modal-body').empty().append(
                    '<h4>Face 1</h4>' +
                    '<img id="fm-1" src="data:image/jpeg;base64,' + loadingImg + '" class="img-fluid">' +
                    '<h4 class="mt-3">Face 2</h4>' +
                    '<img id="fm-2" src="data:image/jpeg;base64,' + loadingImg + '" class="img-fluid">' +
                    '<h3>Face Match: ' + result.score + '</h3>'
                );
...
function generateResult(result) {
        ...
        if (result.hasOwnProperty(side)) {
            if (Object.keys(result[side]).length > 0) {
                if (i === 0) {
                    switch (result.type) {
                        case "BANKCARD":
                            html += "<h3>Bank Card Data</h3>";
                            break;
                        case "DL_PLATE":
                            html += "<h3>USA DL Result</h3>";
                            break;
                        case "BARCODE":
                            html += "<h3>Barcode Data</h3>";
                            break;
                        case "PDF417":
                            html += "<h3>PDF417 Barcode</h3>";
                            break;
                        case "OCR":
                            html += "<h3>Front Side</h3>";
                            break;
                        case "MRZ":
                            html += "<h3>MRZ</h3>";
                            break;
    ...
    if (result.hasOwnProperty("front_img")) {
        html += "<div class='mt-3 d-flex align-items-center'><h4 class='m-0'>FRONT SIDE</h4></div><img id='front-image' src='" + loadingImg + "' class='img-fluid'>";
        getImage('front-image', result.front_img, true);
    }
    if (result.hasOwnProperty("back_img")) {
        html += "<div class='p-2 mt-3 d-flex align-items-center bg-rose'><h4 class='m-0'>BACK SIDE</h4></div><img id='back-image' src='," + loadingImg + "' class='img-fluid'>";
